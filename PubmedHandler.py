...
file_name = 'PubmedHandler_020425.log'
# file_name = None
...
# 2026 baseline
DOWNLOAD_URL = "https://ftp.ncbi.nlm.nih.gov/pubmed/baseline/pubmed26n{:04d}.xml.gz"
OUT_DIR = os.path.expanduser("~/pubmed_baseline_2026")
MAX_ID = 1334 # The largest number of 2026 annual baseline of pubmed

MAX_WORKER = 5 # Adjusted to avoid rate limiting from NCBI

# Cache loaded pubmed abstracts, which should be big
_pmid2abstract = None

# ---------------------------------------------------------------------------
# MongoDB ingestion settings. These read the SAME values the running app reads
# (see reactome_llm/ReactomePubMed.py), so abstracts land where the app looks:
# pubmed_cache.abstracts, with documents shaped {"pmid": str, "abstract": str}.
# ---------------------------------------------------------------------------
load_dotenv()
MONGO_URI = os.getenv("PUBMED_MONGO_URI", "mongodb://localhost:27017")
MONGO_DB = os.getenv("PUBMED_MONGO_DB", "pubmed_cache")
MONGO_COLLECTION = os.getenv("PUBMED_MONGO_COLLECTION", "abstracts")
# Records which baseline files have been fully ingested, so a run can resume.
MONGO_PROGRESS_COLLECTION = "_ingest_progress"
# How many upserts to send to Mongo per network round-trip.
...
def dowload_link(dir_name: str,
    ...
    # Checks if file already exists, skips over previously downloaded files
    if download_path.exists():
        logger.info("Skip {} (already downloaded).".format(link))
        return
...
def persist_abstracts_to_mongo(user: str, password: str):
    # Load pmid2abstract
    pmid2abstract = load_pmid2abstract()
    # Connect to MongoDB
    client = MongoClient('mongodb://{}:{}@localhost:27017/'.format(user, password))
    db_name = 'pubmed_abstract'
    collection_name = 'pmid2abstract'
    db = client[db_name]
    collection = db[collection_name]

    # Create an index on the "pmid" field to ensure uniqueness and improve lookup performance
    ...
    for pmid, abstract in pmid2abstract.items():
        document = {
            'pmid': pmid,  # Integer key (pmid)
            'abstract': abstract  # String value (abstract)
        }
        try:
            ...
            counter += 1
            if counter % 1000 == 0:
                logger.info('Inserted abstracts: {}'.format(counter))
                # break
        except Exception as e:
            logger.error('Failed to insert pmid: {}'.format(pmid))


def extract_and_persist_to_mongo(dir_name: str = OUT_DIR):
    """
    Stream PubMed baseline abstracts straight into MongoDB, one file at a time.

    Unlike extract_all_abstracts() + persist_abstracts_to_mongo(), this never
    holds the whole corpus in memory (the 2026 baseline is ~30-40 GB as a single
    dict, which does not fit in 16 GB RAM). Each pubmed*.xml.gz file is parsed,
    upserted, and recorded as done, so the run is fully resumable: re-running
    skips every file already ingested and continues.

    Writes to MONGO_DB.MONGO_COLLECTION (pubmed_cache.abstracts by default) with
    the schema the app expects: {"pmid": str, "abstract": str}.
    """
    # Echo progress to the terminal too. The module's basicConfig only writes to
    # the .log file (a plain StreamHandler is not a FileHandler), which is why a
    # running job looks "frozen" in the terminal. Add a stdout handler once.
    ...
    baseline_dir = Path(dir_name)
    if not baseline_dir.is_dir():
        raise FileNotFoundError("Baseline directory not found: {}".format(dir_name))
    files = sorted(p for p in baseline_dir.iterdir() if p.name.endswith('.xml.gz'))
    if not files:
        raise FileNotFoundError("No pubmed*.xml.gz files in {}".format(dir_name))

    client = MongoClient(MONGO_URI)
    db = client[MONGO_DB]
    collection = db[MONGO_COLLECTION]
    # Unique index makes the upserts idempotent: re-ingesting a file updates the
    # existing pmids instead of creating duplicates.
    collection.create_index('pmid', unique=True)
    progress = db[MONGO_PROGRESS_COLLECTION]
    done = {doc['_id'] for doc in progress.find({}, {'_id': 1})}

    total_files = len(files)
    logger.info("Ingesting into {} -> {}.{}".format(MONGO_URI, MONGO_DB, MONGO_COLLECTION))
    logger.info("{} baseline files; {} already done, {} to go.".format(
        total_files, len(done), total_files - len(done)))
    ...
    for idx, file_path in enumerate(files, start=1):
        ...
        time_file = time.time()
        try:
            pmid2abstract = extract_abstract(file_path)
            written = _upsert_abstracts(collection, pmid2abstract)
        except Exception as exc:
            # A corrupt/truncated .xml.gz (bad download) would otherwise kill the
            # whole run. Log it, skip it, keep going. It is NOT marked done, so
            # re-downloading that file and re-running will pick it up later.
            bad_files.append(file_path.name)
            logger.warning("[{}/{}] {}: SKIPPED ({}). Re-download to recover.".format(
                idx, total_files, file_path.name, exc))
            continue
        # Mark the file done ONLY after its abstracts are committed, so an
        # interruption mid-file simply re-ingests that one file on the next run.
        progress.update_one({'_id': file_path.name},
                            {'$set': {'_id': file_path.name}},
                            upsert=True)
        logger.info("[{}/{}] {}: {} abstracts ({:.1f}s). Collection total ~{}.".format(
            idx, total_files, file_path.name, written, time.time() - time_file,
            collection.estimated_document_count()))

    if bad_files:
        logger.warning("{} file(s) skipped as corrupt/truncated: {}".format(
            len(bad_files), ', '.join(bad_files)))
    logger.info("Done. ~{} abstracts in {}.{}. Total time: {:.1f} min.".format(
        collection.estimated_document_count(), MONGO_DB, MONGO_COLLECTION,
        (time.time() - time0) / 60))
    client.close()


def _upsert_abstracts(collection, pmid2abstract: dict) -> int:
    """Upsert one file's abstracts in batches. Returns the number written."""
    ops = [UpdateOne({'pmid': pmid},
                     {'$set': {'pmid': pmid, 'abstract': abstract}},
                     upsert=True)
           for pmid, abstract in pmid2abstract.items()]
    written = 0
    for start in range(0, len(ops), MONGO_BATCH_SIZE):
        batch = ops[start:start + MONGO_BATCH_SIZE]
        try:
            result = collection.bulk_write(batch, ordered=False)
            written += result.upserted_count + result.modified_count
        except BulkWriteError as bwe:
            # Ignore harmless duplicate-key errors (code 11000) on resume; raise
            # anything else.
...
#Step 1: Download
#Step 2: Extract all Abstracts
#Step 3: Persist Abstracts to Mongo
if __name__ == '__main__':
    # embed_abstracts_via_ray()
    # download()  # Step 1: download baseline .xml.gz files (already done)
    # extract_all_abstracts()  # Old in-memory pkl build; OOMs on 16 GB RAM. Kept for big-RAM machines.
    # persist_abstracts_to_mongo('root', 'macmongodb01')  # Old pkl -> mongo step.
    # New: resumable, memory-safe parse + load straight into MongoDB (Steps 2 & 3 combined).
    extract_and_persist_to_mongo()
