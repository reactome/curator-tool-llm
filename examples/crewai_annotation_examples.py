...
import sys
sys.path.append('reactome_llm')  # Ensure the reactome_llm package is in the path
...
async def example_basic_annotation():
    ...
    request = AnnotationRequest(
        ...
        # papers=["28754924", "39737163"],  # Local PDFs are resolved as data/papers/<pmid>.pdf
        papers=["28754924"],  # Local PDFs are resolved as data/papers/<pmid>.pdf       
        max_papers=5,
        quality_threshold=0.7,
        enable_full_text=True,  # Enable full-text analysis if available
        enable_literature_search=False,  # Use only the provided PMID list
        schema_path='resources/reactome_domain_model.json'  # Optional schema for validation
    )
...
def create_terminal_output_log_path() -> Path:
    """Create a timestamped output log path in the results directory."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return Path("results") / f"terminal_output_{timestamp}.log"
...
if __name__ == "__main__":
    setup_output_directory()
    log_path = create_terminal_output_log_path()
